...
const EditarReclamo = () => {
  ...
  const handleGuardar = async () => {
    try {
      ...
      navigate("/adminpanel/reclamos/lista");
    } catch (err) {
    ...
    <div className="min-h-screen flex justify-center items-center p-4 sm:p-6">
      <div className="bg-gradient-to-b from-[#002c73] via-[#004c99] to-[#0059b3] rounded-2xl border border-white/20 p-6 sm:p-10 w-full max-w-6xl shadow-xl backdrop-blur-md">
        ...
        <div className="mb-4 space-y-2">
          <p>
            <strong className="text-white/80">Número de Reclamo:</strong>{" "}
            {reclamo.numeroReclamo}
          </p>
          <p>
            <strong className="text-white/80">Nombre:</strong> {reclamo.nombres}{" "}
            {reclamo.apellido}
          </p>
          <p>
            <strong className="text-white/80">Asunto:</strong>{" "}
            {reclamo.asunto || "-"}
          </p>
        </div>
        ...
        <div className="mb-6">
          <label
            className="block mb-2 font-semibold text-white/80"
            htmlFor="estado"
          >
            Estado:
          </label>
        ...
        <div className="mt-6 flex justify-between items-center">
          ...
          <button
            onClick={() => navigate("/adminpanel/reclamos/lista")}
