...
//         try {
//             const respuesta = await axios.get('http://localhost:3000/api/imagenes');
...
//                                 <tr>
//                                     <th className="px-4 py-3 text-left uppercase tracking-wider">Vista previa</th>
//                                     <th className="px-4 py-3 text-left uppercase tracking-wider">Descripción</th>
//                                     <th className="px-4 py-3 text-left uppercase tracking-wider">Acciones</th>
//                                 </tr>
//                             </thead>
//                             <tbody className="bg-white/5 divide-y divide-white/10">
//                                 {imagenes.length > 0 ? imagenes.map((imagen) => (
//                                     <tr className="hover:bg-white/10 transition" key={imagen.id}>
...
//                                             />
//                                         </td>
//                                         <td className="px-4 py-4 whitespace-nowrap text-white/90">{imagen.descripcion}</td>
//                                         <td className="px-4 py-4 whitespace-nowrap">
//                                             <button
...
//                                 )) : (
//                                     <tr>
//                                         <td colSpan="3" className="text-center py-4 text-white/60">No hay imágenes cargadas</td>
//                                     </tr>
//                                 )}
//                             </tbody>
//                         </table>
...
function Imagenes() {
    ...
    const cargarImagenes = async () => {
        try {
            const respuesta = await axios.get('http://localhost:3000/api/imagenes/all');
            if (respuesta.status === 200 && respuesta.data?.body) {
                setImagenes(respuesta.data.body);
            } else {
                console.error("Error en la estructura de la respuesta");
    ...
    const imagenesPaginadas = imagenes.slice(indicePrimeraImagen, indiceUltimaImagen);
                ...
                <div className="overflow-x-auto">
                    {loading ? (
                        ...
                        <table className="min-w-full text-sm divide-y divide-white/20 text-white">
                            ...
                            <tbody className="bg-white/5 divide-y divide-white/10">
                                {imagenesPaginadas.length > 0 ? imagenesPaginadas.map((imagen) => (
                                    <tr className="hover:bg-white/10 transition" key={imagen.id}>
                                        <td className="px-4 py-4 whitespace-nowrap">
                                            <img
                                                src={`http://localhost:3000${imagen.imagen_url}`}
                ...
                {imagenes.length > imagenesPorPagina && (
                    <div className="flex justify-center gap-3 mt-6 text-white">
                        <button
                            ...
                            className={`px-3 py-1 rounded-md ${paginaActual === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/20'}`}
                        >
                            Anterior
                        </button>
                        {[...Array(totalPaginas)].map((_, i) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`px-3 py-1 rounded-md ${num === paginaActual ? 'bg-white/30' : 'hover:bg-white/20'}`}
                                >
                                    {num}
                                </button>
                            );
                        ...
                        <button
                            ...
                            className={`px-3 py-1 rounded-md ${paginaActual === totalPaginas ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/20'}`}
                        >
                            Siguiente
                        </button>
                    </div>
