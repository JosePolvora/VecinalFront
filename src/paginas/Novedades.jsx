...
const Novedades = () => {
  ...
  const novedadesPaginadas = novedades.slice(indicePrimeraNovedad, indiceUltimaNovedad);
      ...
      {novedades.length === 0 ? (
        ...
        <>
          <ul className="space-y-8">
            {novedadesPaginadas.map(({ id, titulo, descripcion, fecha, imagen_url }) => (
              <li
                key={id}
                className="bg-white rounded-xl shadow-md flex gap-6 p-5 hover:shadow-xl transition-shadow"
              >
                <img
                  src={imagen_url ? `http://localhost:3000${imagen_url}` : "https://via.placeholder.com/150"}
                  alt={titulo}
                  className="w-36 h-24 object-cover rounded-md flex-shrink-0"
                />
                <div className="flex flex-col justify-between">
                  <h2 className="text-xl font-semibold text-[#00527A] mb-2">{titulo}</h2>
                  <p className="text-gray-700 line-clamp-3 mb-3">{descripcion}</p>
                  <p className="text-sm text-gray-400">{new Date(fecha).toLocaleDateString("es-AR")}</p>
                </div>
              </li>
            ))}
          </ul>
            ...
            {[...Array(totalPaginas)].map((_, i) => (
              <button
                ...
                className={`px-4 py-1 border rounded ${
                  paginaActual === i + 1 ? "bg-[#00527A] text-white" : "hover:bg-gray-200"
                } transition`}
              >
                {i + 1}
              </button>
